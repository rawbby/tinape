...
// clang-format off
namespace internal{enum class IsDynamicCircleHelper_{};}
// clang-format on

template<typename T>
concept IsDynamicCircle = std::is_same_v<typename T::TypeId, internal::IsDynamicCircleHelper_>;

template<typename P, typename R, typename V>
struct DynamicCircle
{
  using TypeId = internal::IsDynamicCircleHelper_;
  using Position = P;
  using Radius = R;
  using Velocity = V;
  ...
  [[nodiscard]] constexpr auto r45bb(auto sqrt2ub) const
  {
...
constexpr bool
Overlap(IsDynamicCircle auto d0, IsDynamicCircle auto d1) noexcept
{
...
constexpr bool
Overlap(IsCircle auto d0, IsDynamicCircle auto d1) noexcept
{
  // Equation to solve for t:
...
constexpr bool
Overlap(IsDynamicCircle auto d0, IsCircle auto d1) noexcept
{
  return Overlap(d1, d0);
}

constexpr auto
Advance(IsDynamicCircle auto dynamic) noexcept
{
...
constexpr auto
Advance(IsDynamicCircle auto dynamic, auto dt) noexcept
{
